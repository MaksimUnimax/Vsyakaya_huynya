...
assert.match(worker, /DeliveryPolicy\.isPausedFailure\(existingOutbox\)/);
assert.match(worker, /clearOutbox\(key, existingOutbox\.delivery_id\)/);
assert.match(worker, /clearOperation\(key, existingOutbox\.operation_id\)/);
