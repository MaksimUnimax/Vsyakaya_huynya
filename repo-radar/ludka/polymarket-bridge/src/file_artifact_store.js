(() => {
  ...
  const DB_NAME = "pmb_delivery_artifacts_v1";
  const DB_VERSION = 1;
  ...
  function openDb() {
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = () => {
        ...
        if (!chunks.indexNames.contains(CHUNK_ARTIFACT_INDEX)) chunks.createIndex(CHUNK_ARTIFACT_INDEX, "artifact_key", { unique: false });
      };
  ...
  async function stageTextArtifact({ artifactKey, deliveryId, filename, text, mimeType = "application/json;charset=utf-8", chunkCharacters = DEFAULT_CHUNK_CHARACTERS } = {}) {
    ...
    let totalBytes = 0, charOffset = 0, index = 0, pending = [];

    try { await deleteArtifact(key); } catch {}

    await transaction(META_STORE, "readwrite", (tx) => tx.objectStore(META_STORE).put({
      artifact_key: key,
      delivery_id: delivery,
      filename: String(filename || "polymarket-result.json").slice(0, 180),
      mime_type: String(mimeType || "application/json;charset=utf-8"),
      byte_length: 0,
      chunk_count: 0,
      chunk_manifest: [],
      status: "staging",
    ...
    try {
      while (charOffset < source.length) {
        ...
        if (!encoded || encoded.read <= 0) fail("ARTIFACT_ENCODING_STALLED", `TextEncoder stalled at chunk ${index}.`);
        const bytes = scratch.slice(0, encoded.written);
        const sha256 = await sha256Hex(bytes);
        manifest.push({ chunk_index: index, byte_length: bytes.byteLength, sha256 });
        pending.push({
          chunk_key: `${key}:${index}`,
          artifact_key: key,
          delivery_id: delivery,
      ...
      return Object.freeze({ ...meta, chunk_manifest: Object.freeze(manifest.map((item) => Object.freeze({ ...item }))) });
    } catch (error) {
      try { await deleteArtifact(key); } catch {}
      throw error;
    }
  ...
  async function getChunk(artifactKey, chunkIndex) {
    ...
    if (!key || !Number.isSafeInteger(index) || index < 0) fail("ARTIFACT_CHUNK_INDEX_INVALID");
    const db = await openDb();
    try {
      ...
      return { artifact_key: key, chunk_index: index, byte_length: bytes.byteLength, sha256: String(record.sha256 || ""), bytes };
    } finally { try { db.close(); } catch {} }
  }

  async function cleanupDescriptors(descriptors = []) {
    for (const descriptor of Array.isArray(descriptors) ? descriptors : []) {
      try { await deleteArtifact(descriptor?.artifact_key); } catch {}
    }
  }
  ...
  async function cleanupExpired() {
    ...
    for (const meta of metas || []) {
      if (Number(meta?.expires_at_ms || 0) <= current) {
        try { await deleteArtifact(meta.artifact_key); } catch {}
      }
    }
  }

  globalThis.PMBFileArtifactStore = Object.freeze({
    DB_NAME, DB_VERSION, META_STORE, CHUNK_STORE, DEFAULT_CHUNK_CHARACTERS, MAX_CHUNK_BYTES, ARTIFACT_TTL_MS,
    stageTextArtifact, getChunk, deleteArtifact, cleanupDescriptors, cleanupExpired, sha256Hex
  });
})();
